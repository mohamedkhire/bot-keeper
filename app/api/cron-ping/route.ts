...
async function pingUrl(url: string): Promise<{ isOnline: boolean; responseTime?: number }> {
  try {
    ...
    try {
      ...
      logger.info(`[CRON] Pinged ${url} with HEAD - Status: ${response.status} - Response time: ${responseTime}ms`)
      return {
      ...
      logger.warn(
        `[CRON] HEAD request failed for ${url}, trying GET: ${
      ...
      logger.info(`[CRON] Pinged ${url} with GET - Status: ${response.status} - Response time: ${responseTime}ms`)
      return {
        isOnline: response.status < 400,
        responseTime,
      }
    ...
    logger.error(`[CRON] Error pinging ${url}:`, error instanceof Error ? error.message : "Unknown error")
...
export async function GET() {
  try {
    logger.info("[CRON] Server-side ping job started at:", new Date().toISOString())
    ...
    if (enabledProjects.length === 0) {
      logger.info("[CRON] No enabled projects to ping")
    ...
    logger.info(`[CRON] Found ${enabledProjects.length} enabled projects to ping`)

    // Ping each enabled project directly
    const results = await Promise.all(
      enabledProjects.map(async (project) => {
        try {
          logger.info(`[CRON] Pinging project: ${project.name} (${project.url})`)
          ...
          try {
            ...
            logger.error(`[CRON] Invalid URL format for project ${project.id}: ${project.url}`)
          ...
          logger.info(`[CRON] Ping result for ${project.name}: ${isOnline ? "online" : "offline"}`)
          ...
          logger.error(`[CRON] Error pinging project ${project.id}:`, error)
    ...
    logger.info("[CRON] Server-side ping job completed at:", new Date().toISOString())

    return NextResponse.json({
      success: true,
      message: "Server-side ping job executed successfully",
      timestamp: new Date().toISOString(),
      count: results.length,
    })
  } catch (error) {
    logger.error("[CRON] Error in server-side ping job:", error)
    return NextResponse.json(
      {
        success: false,
        error: `Failed to execute server-side ping job: ${error instanceof Error ? error.message : "Unknown error"}`,
        timestamp: new Date().toISOString(),
      },
      { status: 500 },
    )
