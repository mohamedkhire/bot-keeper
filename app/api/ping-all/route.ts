...
async function pingUrl(url: string): Promise<{ isOnline: boolean; responseTime?: number }> {
  try {
    ...
    try {
      ...
      logger.info(`Pinged ${url} with HEAD - Status: ${response.status} - Response time: ${responseTime}ms`)
      return {
      ...
      logger.info(`Pinged ${url} with GET - Status: ${response.status} - Response time: ${responseTime}ms`)
      return {
        isOnline: response.status < 400,
        responseTime,
      }
    ...
    logger.error(`Error pinging ${url}:`, error instanceof Error ? error.message : "Unknown error")
...
export async function GET() {
  try {
    logger.info("Server-side ping job started at:", new Date().toISOString())
    ...
    logger.info(`Found ${enabledProjects.length} enabled projects to ping`)

    // Ping each enabled project directly
    const results = await Promise.all(
      enabledProjects.map(async (project) => {
        try {
          logger.info(`Pinging project: ${project.name} (${project.url})`)
          ...
          try {
            ...
            logger.error(`Invalid URL format for project ${project.id}: ${project.url}`)
          ...
          logger.info(`Ping result for ${project.name}: ${isOnline ? "online" : "offline"}`)
          ...
          logger.error(`Error pinging project ${project.id}:`, error)
    ...
    logger.info("Server-side ping job completed at:", new Date().toISOString())
    ...
    logger.error("Error in server-side ping job:", error)
    return NextResponse.json(
      {
        success: false,
